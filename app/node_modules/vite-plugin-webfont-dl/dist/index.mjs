import{ClientRequest as K}from"node:http";import O from"picocolors";import{AxiosError as G}from"axios";var E={injectAsStyleTag:!0,minifyCss:!0,embedFonts:!1,async:!0,cache:!0,proxy:!1,assetsSubfolder:"",throwError:!1},A=(a={})=>({...E,...a});import{env as B,stdout as p}from"node:process";import H from"picocolors";var m=class{setResolvedLogger(e){this.resolvedLogger=e}isTty(){return p.isTTY&&!B.CI}info(e,t=!0){var s;this.clearLine(),(s=this.resolvedLogger)==null||s.info((t?this.prefix():"")+e)}error(e,t=!0){var s;this.clearLine(),(s=this.resolvedLogger)==null||s.error((t?this.prefix():"")+e)}clearLine(){this.isTty()&&(p.clearLine(0),p.cursorTo(0))}flashLine(e,t=!0){this.isTty()?(this.clearLine(),e=(t?this.prefix():"")+e,e.length<p.columns?p.write(e):p.write(e.substring(0,p.columns-1)),this.flashTimeout&&clearTimeout(this.flashTimeout),this.flashTimeout=setTimeout(()=>{this.clearLine()},500)):this.info(e,t)}prefix(){return H.dim("[webfont-dl] ")}};import{Axios as U,isAxiosError as k}from"axios";import{Agent as M}from"node:http";import{Agent as j}from"node:https";import u from"picocolors";var v=class{constructor(e,t){this.options=e;this.logger=t;this.userAgentWoff2="Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/77.0.0.0 Safari/537.36";this.maxTries=3;this.timeout=2500;this.waitBeforeRetry=[25,2500];this.axios=new U({timeout:this.timeout,proxy:this.options.proxy,httpAgent:new M({keepAlive:!0,family:4}),httpsAgent:new j({keepAlive:!0,family:4})})}async download(e,t,s=1){try{let r=await this.toRequest(e,t);return s>1&&this.logger.info(u.green(`\u2713 ${e}`)+" "+u.dim(`(try #${s})`)),r}catch(r){if(this.logger.error(u.red(`\u2717 ${e}`)+" "+u.dim(`(try #${s})`)+": "+(k(r)?r.message:r)),s<this.maxTries)return await new Promise(i=>setTimeout(i,this.randomWaitInterval())),this.download(e,t,s+1);throw r}}toRequest(e,t){return this.axios.get(e,{headers:{"User-Agent":this.userAgentWoff2},responseType:t!=null?t:"arraybuffer"})}randomWaitInterval(){return Math.floor(Math.random()*(this.waitBeforeRetry[0]-this.waitBeforeRetry[1]+1)+this.waitBeforeRetry[1])}};import{Buffer as W}from"node:buffer";import{create as q,clearCacheById as z}from"flat-cache";var L="3.10.3";var w=class{constructor(e){this.enabled=!0;this.cacheID=`plugin-webfont-dl_${L}.json`;this.hits={css:0,font:0};e.cache===!1&&(this.enabled=!1),this.cache=q({cacheId:this.cacheID,cacheDir:"node_modules/.vite/cache"}),this.enabled||this.clear()}get(e,t){if(!this.enabled)return;let s=this.cache.get(t);if(s)return e==="css"?this.hits.css++:this.hits.font++,s.type==="Buffer"?W.from(s.data):s}save(e,t,s){this.enabled&&(this.cache.set(t,s),this.cache.save(!0))}clear(){z(this.cacheID)}};import{URL as N}from"node:url";import V from"clean-css";var b=class{constructor(e,t,s,r){this.options=e;this.logger=t;this.downloader=s;this.fileCache=r;this.fontUrlRegex=/[-a-z0-9@:%_+.~#?&/=]+\.(?:woff2?|eot|ttf|otf|svg)/gi}async loadAll(e,t){let s="";for(let r of e){let i=await this.load(r),o=this.normalizeUrls(i.trim(),r);s+=o+`
`}return this.formatCss(s,t)}formatCss(e,t){return!t&&this.options.minifyCss?this.minify(e):e.trim()}minify(e){return new V().minify(e).styles}normalizeUrls(e,t){return e=e.replaceAll(this.fontUrlRegex,s=>s.startsWith("http://")||s.startsWith("https://")?s:s.startsWith("//")?"https:"+s:new N(s,t).href),e}async load(e){this.logger.flashLine(e);let t=this.fileCache.get("css",e);if(t)return t;let s=await this.downloader.download(e,"text");return this.fileCache.save("css",e,s.data),s.data}};import{createHash as _}from"node:crypto";var y=class{constructor(){this.fontSrcRegex=/(?:https?:)?\/\/[-a-z0-9@:%_+.~#?&/=]+\.(?:woff2?|eot|ttf|otf|svg)/gi;this.googleFontsKitSrcRegex=/https:\/\/fonts\.gstatic\.com\/l\/font\?kit=[a-z0-9&=_-]+/gi;this.fontFilenameRegex=/[^/]+\.(?:woff2?|eot|ttf|otf|svg)/i;this.googleFontsFileRegex=/\?kit=([a-z0-9_-]+)/i;this.webfontProviders=[/https:\/\/fonts\.googleapis\.com\//i,/https:\/\/fonts\.gstatic\.com\//i,/https:\/\/fonts\.bunny\.net\//i,/https:\/\/api\.fontshare\.com\//i]}parse(e,t,s){var f,c;let r=new Map,i=e.matchAll(this.fontSrcRegex),o=e.matchAll(this.googleFontsKitSrcRegex);if(i)for(let d of i){let g=d.toString(),n=g.match(this.fontFilenameRegex);if(n){let l=n[0];r.set(l,{url:g,filename:l,localPath:t+(s?s+"/":"")+l})}}if(o)for(let d of o){let g=d.toString(),n=(c=(f=g.match(this.googleFontsFileRegex))==null?void 0:f[1])==null?void 0:c.toString();if(n){n.length>50&&(n=_("sha1").update(n).digest("hex"));let l=n+".woff2";r.set(l,{url:g,filename:l,localPath:t+(s?s+"/":"")+l})}}return r}parseBundleCss(e,t,s){let r=new Map,i=new Set([]),o=[],f=/@import\s*(?:url\()?['"]?([^\s'"]+)['"]?\)?;/g,c=/@font-face\s*{[^}]*}/g,d=[...e.matchAll(f)];return[...e.matchAll(c)].forEach(n=>{let l=n[0];this.parse(l,t,s).forEach(D=>{this.webfontProviders.some(P=>P.test(D.url))&&(r.set(D.filename,D),o.push(n[0]))})}),d.forEach(n=>{let l=n[1];this.webfontProviders.some($=>$.test(l))&&(i.add(l),o.push(n[0]))}),{fonts:r,webfontUrlsCss:i,matchedCssParts:o}}};var T=(o=>(o.woff2="font/woff2",o.woff="font/woff",o.ttf="font/ttf",o.otf="font/otf",o.svg="image/svg+xml",o.eot="application/vnd.ms-fontobject",o))(T||{}),F=class{constructor(e){this.options=e}transform(e,t){return t.forEach(s=>{if(!this.options.embedFonts||!s.binary)e=e.replaceAll(s.url,s.localPath);else if(s.binary){let r=s.url.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),i=new RegExp(`url\\(['"]?\\b${r}\\b['"]?\\)`,"gi");e=e.replaceAll(i,`url(data:${this.getFontMime(s)};base64,${s.binary.toString("base64")})`)}}),e}getFontMime(e){let t=e.filename.replace(/^.+\.(.+)$/,"$1");return T[t]}};var x=class{constructor(e){this.options=e}injectAsStylesheet(e,t,s){return this.options.async?this.injectAsync(e,t,s):this.injectSync(e,t,s)}injectAsStyleTag(e,t){return this.options.minifyCss?e.replace(/(\n?)([ \t]*)<\/head>/,`$1$2$2<style>${t}</style>$1$2</head>`):e.replace(/([ \t]*)<\/head>/,`$1$1<style>
${t.replace(/^/gm,"$1$1$1")}
$1$1</style>
